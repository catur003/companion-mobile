...
import { listContainerDatabases, deleteContainerDatabase, createDatabaseSchema, ContainerDatabaseEntry } from '@/lib/companionApi';
 ...
 * kepisah total, bisa ditambah/dihapus dari sini. Istilah "schema" gak
 * dipakai di UI sama sekali, walau di balik layar itu tetep istilah MySQL-nya.
 */
  ...
  const addMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      Alert.alert(
        ...
        `User "${res.newUser}" udah otomatis punya akses ke database ini - langsung bisa di-browse/query dari sini juga.\n\nBuat dipakai app: susun DATABASE_URL manual (host/port sama kayak container ini, nama-db/user/password yang baru dibuat), tambahin ke "Kelola Mapping Project".`
      );
    },
    onError: (err) => {
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      <Card style={styles.introCard}>
        <Text style={styles.intro}>
          ...
          gak perlu bikin container baru tiap project.
        </Text>
      </Card>
...
function DatabaseRow({ db, onDelete, deleting }: { db: ContainerDatabaseEntry; onDelete: () => void; deleting: boolean }) {
  return (
    <Card style={styles.dbRow}>
      <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  dbRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm },
