  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return createCoolifyMysqlDatabase({
        ...
        // Kosong = biarin Coolify generate sendiri (lebih aman, lihat
        // DEPLOYMENT-NOTES.md - password gak bisa diedit lewat form lagi
        // setelah container pertama kali start).
    ...
    onSuccess: (created) => {
      Alert.alert(
        'Database Dibuat',
        `UUID: ${created.uuid}\n\nAmbil connection string dari "MySQL URL (internal)" di dashboard Coolify - jangan ketik manual. Kalau mau dipantau dari ZenVPS (Coolify SQL Query), tambahin databaseUuid ini ke entry project terkait di projects.json (Companion API).`
      );
      router.back();
    },
    ...
    <KeyboardScreen style={styles.screen} contentContainerStyle={styles.content}>
      <Card style={styles.introCard}>
        <Text style={styles.intro}>
          Beta - bikin database MySQL baru langsung ke Coolify. Belum pernah dites ke instance nyata, coba ke
          project kecil dulu.
        </Text>
      </Card>
