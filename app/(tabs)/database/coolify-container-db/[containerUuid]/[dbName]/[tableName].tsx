...
export default function ContainerDbTableDetailScreen() {
  const { containerUuid, dbName, tableName } = useLocalSearchParams<{ containerUuid: string; dbName: string; tableName: string }>();
  ...
  return (
    <View style={styles.screen}>
      ...
      {tab === 'describe' && <DescribeTab containerUuid={containerUuid} dbName={dbName} tableName={tableName} />}
      {tab === 'count' && <CountTab containerUuid={containerUuid} dbName={dbName} tableName={tableName} />}
      {tab === 'preview' && <PreviewTab containerUuid={containerUuid} dbName={dbName} tableName={tableName} />}
    </View>
  );
}
...
function DescribeTab({ containerUuid, dbName, tableName }: { containerUuid: string; dbName: string; tableName: string }) {
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['container-db-describe', containerUuid, dbName, tableName],
    queryFn: async () => {
...
function CountTab({ containerUuid, dbName, tableName }: { containerUuid: string; dbName: string; tableName: string }) {
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['container-db-count', containerUuid, dbName, tableName],
    queryFn: async () => {
      const res = await runCompanionDbQuery(containerUuid, `SELECT COUNT(*) AS total FROM ${ident(tableName)}`, dbName);
...
function PreviewTab({ containerUuid, dbName, tableName }: { containerUuid: string; dbName: string; tableName: string }) {
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['container-db-preview', containerUuid, dbName, tableName],
    queryFn: async () => {
      const res = await runCompanionDbQuery(containerUuid, `SELECT * FROM ${ident(tableName)} LIMIT 10`, dbName);
      return res;
    },
  });
